...
<head>
    ...
    <style>
        ...
        .results-text {
            margin-right: 10px;
            /* Agrega espacio a la derecha del texto */
        }
        ...
        body {
            ...
            /* Cambia 'ruta/a/tu/imagen.jpg' por la ruta de tu imagen */
...
<body>
    ...
    var buscarClienteInput = document.getElementById('buscar_cliente');
    var tablaClientes = document.getElementById('tabla_clientes').getElementsByTagName('tbody')[0];

    buscarClienteInput.addEventListener('input', function() {
        var searchTerm = this.value.trim();
        if (searchTerm === '') {
            tablaClientes.innerHTML = ''; // Limpiar tabla si no hay búsqueda
            return;
        }

        var xhr = new XMLHttpRequest();
        xhr.open('GET', 'buscar_clientes.php?search=' + encodeURIComponent(searchTerm), true);
        xhr.onload = function() {
            if (xhr.status === 200) {
                try {
                    var clientes = JSON.parse(xhr.responseText);
                    tablaClientes.innerHTML = ''; // Limpiar tabla antes de agregar nuevos resultados
                    clientes.forEach(function(cliente) {
                        var row = tablaClientes.insertRow();
                        var cellId = row.insertCell(0);
                        var cellNombre = row.insertCell(1);
                        cellId.textContent = cliente.idClientes;
        ...
        xhr.onerror = function() {
            console.error('Error en la solicitud: se produjo un error al realizar la solicitud AJAX.');
        };
        xhr.send();
    });
});
